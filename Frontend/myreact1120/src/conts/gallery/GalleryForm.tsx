...
const GalleryForm: React.FC = () => {
    ...
    const galleryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        // 선택되었을 때 , 이미지인지 아니면 일반 속성인지를 구별 
        const { name, value, files } = e.target;
        //console.log(`AllNames : ${name} : ${value}`);//모든 타겟의 값이 출력 !
        // files[0] =>첫번째 이미지 객체
        if (name === 'images' && files) {// file이 존재할 경우
            ...
            // name:iamges가 아닐 경우 일반 string 저장 
    ...
    const gallerySubmit = async(e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(`http://192.168.0.195/myictstudy/gallery/add`
